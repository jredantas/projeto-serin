...
public class VehicleSample {
	...
	public static void main(String[] args) throws Exception {
		...
		URL ontology = VehicleSample.class.getClassLoader().getResource(
				URI_ONTOLOGY.substring(URI_ONTOLOGY.indexOf('/') + 1));

		model = ModelFactory.createOntologyModel();
		model.read(ontology.openStream(), null);

		Individual peugeout207 = model.createIndividual(Vehicle.NS + "207",
				Vehicle.VEHICLE);
		peugeout207.setPropertyValue(Vehicle.YEAR,
				model.createTypedLiteral(2010));
		peugeout207.setPropertyValue(Vehicle.MODEL,
				model.createTypedLiteral("207"));
